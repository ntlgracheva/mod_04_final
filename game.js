...
(() => {
    ...
    const gameStart = () => {
        ...
        const passUser = () => {
            ...
            if (!Number.isInteger(userN) || userN > userBalls || userN < 1) { return user = passUser() }

            //**консоль */
            console.log(`число пользователя userN ${userN}`);

            return user = passComp(userN);
        }
        ...
        const passComp = (num) => {
            ...
            if (isOdd === odd) {
                ...
                else { alert("Компьютер угадал!" + `\n шарики пользователя ${userBalls}` + `\n шарики компьютера ${compBalls}`); return user = passUser() }
                ...
                else { alert("Компьютер НЕ угадал!" + `\n шарики пользователя ${userBalls}` + `\n шарики компьютера ${compBalls}`); return user = passUser() }
